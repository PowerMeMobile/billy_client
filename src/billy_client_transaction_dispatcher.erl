...
re_pack(_Container = #svc_container{ details = ListOfPackages }) ->
	DetailsList = lists:map(fun({ SvcTypeID, #svc_details{quantity = Quantity }})->

		#billy_transaction_svc_package{
			svc_type_id = SvcTypeID,
			svc_details = #billy_transaction_svc_details{quantity = Quantity}
			}
